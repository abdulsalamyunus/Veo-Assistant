...
async function generateContent(
  ...
  if (generateAudio) {
    params.config.generateAudio = true;
  }
...
const audioCheckbox = document.querySelector(
  '#audio-checkbox',
) as HTMLInputElement;
...
audioCheckbox.addEventListener('change', () => {
  generateAudio = audioCheckbox.checked;
});
...
function setControlsDisabled(disabled: boolean) {
  ...
  audioCheckbox.disabled = disabled;
}
...
async function generate() {
  ...
  try {
    await generateContent(prompt, base64data, apiKey, generateAudio);
    statusEl.innerText = '';
  } catch (e) {
